...
const MenuBar: React.FC<{ editor: Editor | null }> = ({ editor }) => {
  ...
  const menuItems = useMemo(() => {
    ...
    const createAction = (command: (chain: any) => any, isActiveCheck?: () => boolean | undefined) => ({
      action: () => {
    ...
    return [
      ...
      { action: addImage, icon: ImageIcon, label: 'Add Image', type: "button" },
      ...
      { type: 'divider' as const },
      { ...createAction(chain => chain.unsetHighlight(), () => editor && (!editor.isActive('highlight') || editor.isActive('highlight', {color: ''}))), icon: Highlighter, label: 'No Highlight', type: "button"},
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        ...
        // Ensure core block elements are enabled by StarterKit by default
        // heading: {}, // default levels 1-6
        // blockquote: {},
        // bulletList: {},
        // orderedList: {},
        // codeBlock: {},
      ...
      Color, // Enables setting text color
      Highlight.configure({ multicolor: true }), // Enables highlight
      Superscript, // Enables superscript
      Subscript,   // Enables subscript
    ],
    content: value || '',
  ...
  useEffect(() => {
    if (editor && !editor.isDestroyed && value !== null && value !== undefined) {
      if (value !== editor.getHTML()) {
        ...
        // Crucially, focus the editor after setting content, especially for edit pages
