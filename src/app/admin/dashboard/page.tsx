...
import { Users, FileText, MessageSquare, Tags, Settings, LogOut, PlusCircle, Edit, Eye } from 'lucide-react';
import { MOCK_BLOG_POSTS } from '@/lib/constants';

const stats = [
  { title: 'Total Blogs', value: MOCK_BLOG_POSTS.length, icon: FileText, color: 'text-primary' },
  { title: 'Total Users', value: 150, icon: Users, color: 'text-accent' }, 
  { title: 'Total Comments', value: 450, icon: MessageSquare, color: 'text-green-500' },
  { title: 'Categories', value: 5, icon: Tags, color: 'text-yellow-500' },
];
        ...
        <TranslucentContainer baseColor="card" backgroundOpacity={70} padding="p-6 md:p-8">
          ...
          <div className="mt-6">
            ...
            <p className="text-sm text-muted-foreground">
              The "Create New Post" page uses a basic textarea for content. In a production app, integrate a rich text editor (e.g., Tiptap, Quill, or TinyMCE) for advanced formatting, image uploads, and embeds.
            </p>
          </div>
        ...
        <TranslucentContainer baseColor="card" backgroundOpacity={70} padding="p-6 md:p-8">
          ...
          <div className="space-y-3">
            ...
            <Button variant="outline" asChild className="w-full justify-start">
             ...
             <Button variant="outline" asChild className="w-full justify-start">
              <Link href="/admin/transfer-access"><Eye className="mr-2 h-4 w-4" /> Transfer Admin Access</Link>
            </Button>
